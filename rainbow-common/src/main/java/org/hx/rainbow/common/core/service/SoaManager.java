...
public class SoaManager {
	...
	public  RainbowContext invokeNoTx(RainbowContext context) {
		...
		RainbowContext resultContext = new RainbowContext();
		try {
			resultContext = this.soaInvoker.call(context);
		} catch (Exception e) {
			if ((e instanceof AppException)) {
				throw ((AppException) e);
			}else{
				throw new SysException("rainbowSoa: Service invoker is error!"+e.getMessage(),e.getCause());
			}
		}
